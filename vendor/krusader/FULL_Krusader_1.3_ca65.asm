...
ECHO:	PHA			;PUSH A TO STACK
	JSR COUT		;COUT
  	PLA			;PULL A FROM STACK
	RTS

SHWMSG:	LDY #$0			;INITIALIZE COUNTER
PRINT:	LDA (STRL),Y		;LOAD NEXT CHAR
...
@NEXT:	LDY #$00
	...
	JSR PRNTLN
	JSR UPDTCL
	;LDA KBDRDY   GAK should be moved this to the apple1 block
	.if APPLE1
		BPL @NEXT
	.else
		LDA ACIAStatus ;GAK grab the current status
		AND #1 ;GAK mask for the RDRF bit
		CMP #1 ;GAK check if bit is set
		BNE @NEXT ;GAK change from BEQ to BNE
	.endif
	;LDA KBD  ;GAK removed this
	LDA ACIAData ; GAK - grab that character from the buffer 
@RET:	RTS
...
@DSMBL2:	JSR INSTDSP	; Disassemble and display instr.
	...
	;LDA KBDRDY	; Now disassemble until key press -GAK removed this
...
.else		;L1
		...
		AND #1 ;GAK mask for the RDRF bit
		CMP #1 ;GAK check if bit is set
		BNE @DSMBL2 ;GAK change from BEQ to BNE	
		;BEQ @DSMBL2 - GAK removed
		LDA ACIAData ;GAK pull the stop char from the buffer
.endif		;L0

	;LDA KBD - GAK changing this for ACIAData
...
NUMCMD	=$0D
CMDS:	.byte "NLXEMRDI!$AVP"  ;GAK...was ASCII
...
ERRPRE:	.byte " :RRE"   ;GAK...was .ascii
ERRMSG:
.if UNK_ERR_CHECK	;L1
	.byte "UNK" ;GAK...was .ascii
.endif				;L0
	.byte "MNE" ;GAK...was .ascii
	.byte "ADD" ;GAK...was .ascii
	.byte "SYN" ;GAK...was .ascii
	.byte "OVF" ;GAK...was .ascii
	.byte "SYM" ;GAK...was .ascii
	
MSGSZ = $21
;MSG	.ascii "3.1 NESSEW NEK YB 20C56 REDASURK",CR
MSG:	.byte "3.1 NESSEW NEK YB 20C56 REDASURK"  ;GAK...was .ascii
...
.if MINIMONITOR	;L1
	...
	NREGS	=$5
	DBGCMD:	.byte "PSYXALH"  ;GAK...was .ascii
	NDBGCS	=NREGS+2
	FLAGS:	.byte "CZIDB" ;GAK...was .ascii
		.byte $00	; A non-printing character - this flag always on
		.byte "VN"  ;GAK...was .ascii
