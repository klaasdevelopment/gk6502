  ...
  vrEmu6502MemWrite writeFn)
{
  ...
  VrEmu6502* vr6502 = (VrEmu6502*)calloc(1, sizeof(VrEmu6502));
  if (vr6502 != NULL)
  {
    vr6502->model = model;
...
static void beginInterrupt(VrEmu6502* vr6502, uint16_t addr)
{
  ...
  setBit(vr6502, FlagI);
  vr6502->wai = false;
  if (is65c02(vr6502)) clearBit(vr6502, FlagD);
  vr6502->pc = read16(vr6502, addr);
  vr6502->step = 6; /* interrupt entry consumes seven cycles including this tick */
}
...
static uint16_t ind(VrEmu6502* vr6502)
{
  uint16_t addr = read16(vr6502, vr6502->pc);
  vr6502->pc += 2;
  return is65c02(vr6502) ? read16(vr6502, addr) : read16Bug(vr6502, addr);
}
...
static void adcd(VrEmu6502* vr6502, vrEmu6502AddrModeFn modeAddr)
{
  ...
  unsigned a = vr6502->ac, carry = testBit(vr6502, FlagC);
  unsigned binary = a + value + carry;
  unsigned low = (a & 15) + (value & 15) + carry;
  if (low > 9) low = ((low + 6) & 15) + 16;
  unsigned result = (a & 0xf0) + (value & 0xf0) + low;
  setOrClearBit(vr6502, FlagV, (~(a ^ value) & (a ^ result) & 0x80));
  if (!is65c02(vr6502)) {
    setOrClearBit(vr6502, FlagN, result & 0x80);
    setOrClearBit(vr6502, FlagZ, (binary & 0xff) == 0);
  }
  if (result >= 0xa0) result += 0x60;
  setOrClearBit(vr6502, FlagC, result > 255);
  vr6502->ac = (uint8_t)result;
  if (is65c02(vr6502)) { ++vr6502->step; setNZ(vr6502, vr6502->ac); }
}
...
{
  uint8_t value = vr6502->readFn(modeAddr(vr6502), false);
  int a = vr6502->ac, borrow = !testBit(vr6502, FlagC);
  int binary = a - value - borrow;
  int low = (a & 15) - (value & 15) - borrow;
  int result;
  if (!is65c02(vr6502)) {
    if (low < 0) low = ((low - 6) & 15) - 16;
    result = (a & 0xf0) - (value & 0xf0) + low;
    if (result < 0) result -= 0x60;
    setNZ(vr6502, (uint8_t)binary);
  } else {
    result = binary;
    if (result < 0) result -= 0x60;
    if (low < 0) result -= 6;
  ...
  setOrClearBit(vr6502, FlagV, ((a ^ value) & (a ^ binary) & 0x80));
  setOrClearBit(vr6502, FlagC, binary >= 0);
  vr6502->ac = (uint8_t)result;
}
